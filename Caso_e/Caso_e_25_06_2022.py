...
plt.savefig("Caso_e_Correlacion")
...
plt.tight_layout()
plt.savefig("Caso_e_Histograma y cajas")
...
plt.savefig("Caso_e_Correlacion_dispersion")
plt.show()

# MODELO DE REGRESIÓN LINEAL MÚLTIPLE
## División del conjunto de datos
# X= pd.DataFrame(np.array([X2]).T, columns=['Nadir [Hz]'])
X11= pd.DataFrame(np.array([X2]).T, columns=['RoCoF [Hz/s'])
Y= pd.DataFrame(np.array([X1]).T, columns=['DP [MW]'])
from sklearn.model_selection import train_test_split
X_train, X_test, Y_train, Y_test = train_test_split(X11, Y, random_state = 5, test_size=0.2)
print(X_train)
print(Y_train)
print(X_test)
print(Y_test)
# X1_train, X1_test, Y1_train, Y1_test = train_test_split(X11, Y, random_state = 5, test_size=0.2)
...
x = np.linspace(-0.8, 0, len(X_train))
...
plt.text(-0.75, 0.2, 'R2 = 0,985331', fontsize=16)
plt.text(-0.75, 0.05, '  Y =  -0.0757 - 1,6838 X1  ', fontsize=16)
plt.xlabel('RoCoF [Hz/s]')
plt.ylabel('Potencia de salida DP [MW]')
plt.legend()
plt.savefig("Caso_e_Resultados")
plt.show()




with pd.ExcelWriter('Resultados_caso_e.xlsx', engine='xlsxwriter') as writer:
...
polilínea = np.linspace(-0.8, 0, len(X_train))
plt.scatter(x1, y2, s = 30, color='blue', marker='o',label = "Datos de entrenamiento")
plt.plot (polilínea, modelo (polilínea), color='red', marker='_', label = "Regresión cuadrática")
plt.text(-0.8, 0.2, 'R2 = 0,985352', fontsize=16)
plt.text(-0.8, 0.05, '  Y =  -0,04997 X1^2 - 1,729 X1 - 0,08347  ', fontsize=16)
plt.xlabel('RoCoF [Hz/s]')
plt.ylabel('Potencia de salida DP [MW]')
plt.legend()
