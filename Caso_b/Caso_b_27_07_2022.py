...
"""
# Caso b: 
# 𝑌 ≡ 𝐷𝑃, 𝑋0 ≡ 1, 𝑋1 ≡ 𝑁𝑎𝑑𝑖𝑟, 𝑋2 ≡ df/dt, 𝑋3 ≡ 𝑉𝑒𝑛𝑑
...
plt.savefig("Caso_b_Correlacion")
...
plt.tight_layout()
plt.savefig("Caso_b_Histograma y cajas")
...
plt.savefig("Caso_b_Correlacion_dispersion")
plt.show()

# MODELO DE REGRESIÓN LINEAL MÚLTIPLE
## División del conjunto de datos
X= pd.DataFrame(np.array([X3,X4,X5]).T, columns=['RoCoF [Hz/s]','Vend [kV]','fend [Hz]'])
...
df1 = pd.DataFrame()
df1['Parámetros'] = ['Coeficiente de correlación múltiple R','Coeficiente de determinación R2','Coeficiente de determinación ajustado R2','Error estimado estándar']
...
##↨ Predicción
y_pred = B[0]*np.ones(len(X_test))+B[1]*X_test.iloc[:,0]+B[2]*X_test.iloc[:,1]+B[3]*X_test.iloc[:,2]
...
plt.legend()
plt.savefig("Caso_b_Resultados")
plt.show()




with pd.ExcelWriter('Resultados_caso_b.xlsx', engine='xlsxwriter') as writer:
